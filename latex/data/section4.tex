...
Fig.~\ref{fig:particle_flow} shows how various types of particles interact within the CMS sub-detectors.
...
\begin{figure}[!hbt]
    ...
    \caption{Cross-sectional view of the CMS detector with all of the sub-detectors labeled.The colored lines correspond to different particle species, which interact with different pieces of the detector and may or may not be bent by the magnetic field~\cite{CMSSlice}.}
...
After CHS and the clustering procedure, the momentum and energy of the jets still might not be the same as those from the initial parton, whether because of pileup or detector effects.
...
The goal is to make sure each jet has a relative response ($\mathcal{R}_{rel}=RelRsp=\frac{\ptsup{reco}}{\ptsup{ref}}$) of 1.0, where $\ptsup{reco}$ is the reconstructed jet \pt and $\ptsup{ref}$ is the true or reference \pt of the jet without all of the deleterious effects.
...
A final level of modification to the reconstructed objects is an $\eta$ dependent smearing factor applied to the jet 4-momenta coming from the MC samples.
The distribution of jet energies within the MC simulation tends to be more sharply peaked and less broad than the same distribution in data.
In other words, the MC has a smaller jet energy resolution (JER) than we can realistically measure using the CMS detector.
The deterministic ``smearing'' method recommended by CMS seeks to make the jet energy resolution in MC match the jet energy resolution in data.
The reconstructed jet \pt is scaled by a correction factor $C_{JER}$ as determined in equation~\ref{eq:C_JER}, where $C_{\eta}$ is a correction factor derived as a function of $\eta$ whose values can be found in table~\ref{tab:JER_Ceta}.
The multiplicative JER correction factor is then used to modify the jet 4-momentum as in equation~\ref{eq:Jet_JER}.
\begin{equation}\label{eq:C_JER}
    C_{JER}=max\left(0.0,\frac{p_{T}^{GEN}}{p_{T}^{RECO}}+C_{\eta}\cdot\left(1-\frac{p_{T}^{GEN}}{p_{T}^{RECO}}\right)\right)
\end{equation}
\begin{equation}\label{eq:Jet_JER}
    \textbf{X}_{Jet}^{corrected}=C_{JER}{\cdot}\textbf{X}_{Jet}^{RECO}
\end{equation}
Although the reconstruction of the \VETslash object will not be discussed until section~\ref{sec:MET}, it is important to note that its value is intrinsicaly tied to that of the jets.
Any modification to the jet energies must also be propogated to the \VETslash.
The propogation of the corrections due to the JER scaling is shown in equations~\ref{eq:MET_JER_X} and~\ref{eq:MET_JER_Y}.
The propogation of the JEC, on the other hand, will be discussed in section~\ref{sec:MET}~\cite{JetEnergyResolutionTwiki}.
\begin{equation}
    E_{\mathrm{x}}^{\mathrm{corrected}}\hspace{-3.5em}/\kern3.1em = \left(1-C_{JER}\right)Jet_{x}^{RECO}+E_{\mathrm{x}}^{\mathrm{RECO}}\hspace{-2.7em}/\kern3.1em\label{eq:MET_JER_X}
\end{equation}
\begin{equation}
    E_{\mathrm{y}}^{\mathrm{corrected}}\hspace{-3.5em}/\kern3.1em = \left(1-C_{JER}\right)Jet_{y}^{RECO}+E_{\mathrm{y}}^{\mathrm{RECO}}\hspace{-2.7em}/\kern3.1em\label{eq:MET_JER_Y}
\end{equation}
...
Those coefficients can be found in table~\ref{tab:METPhi_Correction_Coefficients}.
...
These predictions are ensembles of simulated events made by Monte Carlo (MC) event generators which are broken up by physics process and final state and then recombined during the analysis~\cite{Siegert:2010cru,Mangano:2005dj,Dobbs:2004qw}.
...
A snapshop of the conditions at some point in time is called a global tag.
For reference, this analysis uses the GR\_R\_53\_V10 and START53\_V7A global tags for data and simulation, respectively~\cite{JETGlobalTag}.
